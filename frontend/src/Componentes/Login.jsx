...
const Login = () => {
  ...
  const { contraseña, correo } = inputs;
    ...
    if (contraseña !== "" && correo !== "") {
      ...
      await axios
        ...
        .then((res) => {
          ...
          setInputs({ contraseña: "", correo: "" });
          setTimeout(() => {
            setMensaje("");
            navigate('/bienvenida/${data?.usuario.id}');
            setLoading(false);
          }, 3000);
        })
        .catch((error) => {
      ...
      <div className={AppStyles.container}>
        ...
        <div>
          <div className={styles.formContainer}>
            <h2>Accede</h2>
            <h3>Inicia sesión para continuar
            </h3>
            ...
            <form onSubmit={(e) => onSubmit(e)}>
              ...
              {/* Actualizar despues para que inicie sesion con id de usuario
              <button type="submit">{loading ? 'Cargando...' : "Inicia sesión"} </button> */}

              <button type="submit" onClick={() => navigate('/Bienvenida')}>Ingresar </button>
              <p>Aún no tienes una cuenta?
                <b onClick={() => navigate('/registrousuario')}> Registrate </b>
              </p>
